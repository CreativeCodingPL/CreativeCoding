...
void drzewo(float x, float y, float wysokosc, float kat)
{
  ...
  { //lodyga/galazka
    ...
    line(x, y, xk, yk);
    drzewo(xk, yk, wysokosc*0.76-0.6*noise(x,y,frameCount*0.002), kat + 60 + 20*noise(x,y,frameCount*0.001));
    drzewo(xk, yk, wysokosc*0.70-0.6*noise(x,y,frameCount*0.002), kat - 20 - 20*noise(x,y,frameCount*0.001));
  }
  else
  { //lisc
    ellipse(x, y, 5, 5);

...
void setup() {
  ...
  for (int i = 0; i < maxi; i++) {
    if (i<maxi/2) {
      line(-sin( PI*(i-maxi)/maxi*p)*brzeg/q, i*odly+odly/2, 0, i*odly+odly/2); //lewy
      line(brzeg - sin( PI + PI*(i-maxi)/maxi*p)*brzeg/q, i*odly+odly/2, brzeg, i*odly+odly/2); //prawy
      line(-sin( PI*(i-maxi)/maxi*p)*brzeg/q, i*odly+odly/2, 0, i*odly+odly/2); //lewy
      line(brzeg - sin( PI + PI*(i-maxi)/maxi*p)*brzeg/q, i*odly+odly/2, brzeg, i*odly+odly/2); //prawy
    } else {

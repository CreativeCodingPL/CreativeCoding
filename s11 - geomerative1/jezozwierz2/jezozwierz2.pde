  ...
  for( int i = 0; i < grp.children.length; i++ ){
    ...
    for( int j = 0; j < sciezki.length; j ++ ){
      ...
      for( float r = 0; r <= 1.0; r += 0.01 ){
        ...
        float angleNoise = noise( punkt.x / 100.0, punkt.y / 100.0, millis()/ 1500.0 );
        float mappedAngleNoise = map( angleNoise, 0, 1, radians(-30), radians(30) ); 
        
        float angleTransform = ( i < 1 ) ? PI/2 : -PI/2;
        
        float kat = atan2( styczna.y, styczna.x ) + angleTransform + mappedAngleNoise;
